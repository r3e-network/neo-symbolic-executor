...
public class ReviewFixesTests
{
    ...
    public void Readme_DocumentsCoverageIntegrityAndCurrentTooling()
    {
        ...
        // The README must state exactly one current test count. This replaces a ~500-line hardcoded
        // staleness blocklist: a single regex both pins the count and rejects any stale count, with no
        // per-commit blocklist churn. When the suite size changes, update this value and the README.
        var testCountClaims = Regex.Matches(readme, @"(\d+) xUnit cases passing");
        testCountClaims.Should().ContainSingle("the README states exactly one xUnit test count");
        testCountClaims[0].Groups[1].Value.Should().Be("1421");
    ...
    [Fact]
    public void Expr_IdentityFoldingCanonicalizesBalanceDeltaShapes()
    {
        // The NEP balance_delta obligation (FormalVerifier.ValueMatchesBalanceDelta) is a syntactic AST
        // matcher that relies on the engine constant-folding identity-wrapped arithmetic so an
        // identity-wrapped delta reaches it already canonical. Pin the folds it depends on, so a
        // refactor that dropped them (re-introducing a false-positive Violated on `balance - (amount+0)`)
        // would fail here.
        var balance = Expr.Sym(Sort.Int, "storage_value_16");
        var amount = Expr.Sym(Sort.Int, "amount");
        Expr.Sub(balance, Expr.Add(amount, Expr.Int(0))).Should().Be(Expr.Sub(balance, amount));
        Expr.Sub(balance, Expr.Add(Expr.Int(0), amount)).Should().Be(Expr.Sub(balance, amount));
        Expr.Add(balance, Expr.Mul(amount, Expr.Int(1))).Should().Be(Expr.Add(balance, amount));
        Expr.Add(balance, Expr.Mul(Expr.Int(1), amount)).Should().Be(Expr.Add(balance, amount));
    }
